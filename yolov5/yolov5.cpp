...
ICudaEngine* createEngine_s(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../yolov5s_ai_tracks.wts");
...
int main(int argc, char** argv) {
    ...
    std::string wts_name="../yolov5s.wts";
...
//   char* saving_folder;
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./yolov5 -d rtsp://stream-url  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    cv::VideoCapture capture(stream_url);
    if( !capture.isOpened() )
        throw "Error when reading stream";

    cv::Mat frame;

   // cv::VideoCapture capture("sample.mp4");  //if trying to open a video from the same directory          
   // if (!writer.isOpened()) {
   ...
   // }
    ...
    std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(endreading - startreading).count() << "ms while reading" << std::endl;
    if( !capture.isOpened() )
    ...
    if(a != b ){ //trick to not having problem with zmq as the compilation takes place twice.
	 ...
	 //setting up the ZMQ context and socket variables <<<ZMQ>>>>
    	...
    	for( ; ; )
    	{
        	...
        	fcount++;
        	afcount++;
        	std::cout << "frame n" << afcount<< std::endl;
        	capture >> frame;
        	if(frame.empty())
            	   break;
        	for (int b = 0; b < fcount; b++) {
            	cv::Mat pr_img = preprocess_img(frame,INPUT_H,INPUT_W); // letterbox BGR to RGB
            	int i = 0;
	...
	std::cout << "frame n" << afcount<< std::endl;
        ...
        std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(endinference - startinference).count() << "ms while inference" << std::endl;
        std::vector<std::vector<Yolo::Detection>> batch_res(fcount);

        auto startwriting = std::chrono::system_clock::now();
	...
	for (int b = 0; b < fcount; b++) {
		...
		auto& res = batch_res[b];
            	//std::cout << "total elements in the frame: " << res.size() << std::endl;
            	cv::Mat img = frame;
            	json_object *mainobj = json_object_new_object();
       		json_object *frameWidth = json_object_new_int(img.rows);
		json_object *frameHeight = json_object_new_int(img.cols);
		json_object *positions = json_object_new_object();
		json_object *detectionsArray = json_object_new_array();
		...
		json_object_object_add(size,"w", frameWidth);
		json_object_object_add(size,"h", frameHeight);
		
		//std::vector<bool> visited(res.size(), false);

		json_object *lat = json_object_new_double (-67.8916);
		json_object *lon = json_object_new_double (32.1322);

		json_object_object_add(location,"lat", lat);
		json_object_object_add(location,"lon", lon);

		json_object_object_add(source,"location camera", location);
            	json_object_object_add(source,"size frame", size);
			
		for (size_t j = 0; j < res.size(); j++) {


                	//std::cout << "element's class :" << res[j].class_id;
			//std::cout << "confidence:" << res[j].conf;
                	...
                	//        if (res[j].class_id == res[i].class_id) {
                	//        visited[i] = true;
               	 	//        count++;
                	//        }
                	//}
		
            		json_object *detectionObj = json_object_new_object();
	
			json_object *objectName = json_object_new_string(classes[res[j].class_id].c_str());
			json_object *objectConfidence = json_object_new_double(res[j].conf);
		
                	json_object_object_add(detectionObj,"name", objectName);
                	json_object_object_add(detectionObj,"confidence", objectConfidence);
			//json_object_object_add(detectionobj, classes[res[j].class_id], counter);


                	cv::Rect r = get_rect(img, res[j].bbox);
               		cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
                	json_object *ulx = json_object_new_int(r.x);
			json_object *uly = json_object_new_int(r.y);
			json_object *h = json_object_new_int(r.height);
			json_object *w = json_object_new_int(r.width);
			//std::cout << "rectangle x :" << r.x;
			//std::cout << "rectangle y :" << r.y;
			//std::cout << "rectangle height: " << r.height;
			//std::cout << "rectangle width : " << r.width;
			cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
	 		json_object_object_add(positions,"ulx", ulx);
			json_object_object_add(positions,"uly", uly);
			json_object_object_add(positions,"w", w);
			json_object_object_add(positions,"h", h);


			json_object_object_add(detectionObj,"positions", positions);

			json_object_array_add(detectionsArray, detectionObj); 
            	}


	    	json_object_object_add(mainobj,"detections", detectionsArray);
	    	json_object_object_add(mainobj,"source", source);
		
		printf("jobj from str:\n---\n%s\n---\n", json_object_to_json_string_ext(mainobj, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY));
            	//FRAME will be used to be sent with ZMQ
	   
	    	std::string jsonStringToSend =  json_object_to_json_string_ext(mainobj, JSON_C_TO_STRING_SPACED | JSON_C_TO_STRING_PRETTY);
	    	cv::resize(frame, frame, cv::Size(), 0.25, 0.25);
	       	
	    	std::vector<uchar> buffer;
	    	cv::imencode(".jpg", frame, buffer);
            	zmq_send(publisher, buffer.data(), buffer.size(), ZMQ_SNDMORE);
	    
	    	const void * toSend = jsonStringToSend.c_str();	    
	    	zmq_send(publisher, toSend, jsonStringToSend.size(), 0);
	    	gst_udpsink.write(frame);
           	// cv::imwrite("../samples"+ std::to_string(afcount) + ".png", frame); //For writing the frames on the local memory. 
        
	}
       	
	auto endwriting = std::chrono::system_clock::now();
        std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(endwriting - startwriting).count() << "ms while writing" << std::endl;
